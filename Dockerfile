...
sed -i -e 's/tls_cert_file: \/etc\/pki\/cyrus-imapd\/cyrus-imapd.pem/tls_cert_file: \/etc\/pki\/tls\/certs\/server.crt/' /etc/imapd.conf && \
sed -i -e 's/tls_key_file: \/etc\/pki\/cyrus-imapd\/cyrus-imapd.pem/tls_key_file: \/etc\/pki\/tls\/certs\/server.key/' /etc/imapd.conf && \
...
CMD [/bin/bash -c "/sbin/service saslauthd start && /sbin/service postfix start && /sbin/service cyrus-imapd start && /bin/bash"]
